...
curr=os.path.join(os.getcwd(),'dataset/data/'+opt.dataset+'/image_SRF_'+str(opt.upscale_factor))
for i in os.listdir(curr):
    if i[5:7]== opt.model:
        if i[14:16]== "LR":
            img = Image.open(os.path.join(curr,i)).convert('YCbCr')
            img_bicubic = Image.open(os.path.join(curr,i))
        else:
            img_hr=Image.open(os.path.join(curr,i))
y, cb, cr = img.split()
